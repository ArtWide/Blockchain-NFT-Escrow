...
class Event_Filter():
    ...
    def handle_migration(self, event):
        ...
        token_ids = event['args']['tokenIds']
        migration_time = event['args']['timestamp']
        transaction_hash = event['transactionHash'].hex()
        print(f"user address: {user}, token ID: {token_ids}, timestamp: {migration_time}, transaction hash: {transaction_hash}")
    ...
    def handle_withdrawal(self, event):
        ...
        transaction_hash = event['transactionHash'].hex()
        print(
            f"user address: {user}, token ID: {token_ids}, timestamp: {withdrawal_time}, transaction hash: {transaction_hash}")
...
# NETTYPE = 'testnet'
BLOCK_SIZE = 1500
CONTRACT_ADDRESS = '0xdcD9AC58c27eB843Be624Cc8f889cb9308423f4b'
...
while True:
    for NET in NETWORK:
        try:
            ...
            ABI_FILE = 'NFT-Edition-Migration'
